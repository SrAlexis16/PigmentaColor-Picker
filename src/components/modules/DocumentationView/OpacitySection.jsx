...
const OpacitySection = ({ isOpen, onOpenChange }) => {
  const { t } = useTranslation();
  const articles = [
    {
      title: t("content.articles.opacityTitle"),
      description: t("content.articles.opacityDescription"),
      url: 'https://developer.mozilla.org/en-US/docs/Web/CSS/opacity',
    },
    {
      title: t("content.articles.cssOpacityTitle"),
      description: t("content.articles.cssOpacityDescription"),
      url: 'https://css-tricks.com/almanac/properties/o/opacity/',
    },
  ];
    ...
    >
      <CollapsibleTrigger asChild>
        <Button className="flex items-center justify-between cursor-pointer px-2 py-1 w-full bg-transparent border-none">
          <h1 className="text-neutral-200 dark:text-black text-md lg:text-2xl font-semibold">{t('content.opacity.title')}</h1>
          {isOpen ? (
      ...
      <CollapsibleContent className="px-2 py-2 text-lg text-neutral-300 dark:text-black">
        <p className="text-sm mb-2">
          {t('content.opacity.description1')}
        </p>
        <p className="text-sm">
          {t('content.opacity.description2')}
        </p>
        <section className="mt-4">
          <h4 className="text-md font-semibold mb-2">{t('content.opacity.title')}</h4>
